...
<style>
#preorder{box-sizing:border-box;position:relative;width:100vw;overflow-x:hidden;background:linear-gradient(rgba(9,23,7,.72),rgba(9,23,7,.82)),url(${BASE}assets/bg/overgrown-sphere-as470076105.webp) center/cover no-repeat fixed;padding-top:clamp(140px,12vw,180px);color:#fffffe;font-family:'Almarai',sans-serif;letter-spacing:.03em;padding-bottom:110px}
...
#preorder .po-format h3{font-family:'Almarai',sans-serif!important;font-weight:800;font-size:14px;letter-spacing:.12em;text-transform:uppercase;color:#fd7547;margin:12px 0 0}
...
(function () {
  function fit() {
    var e = document.getElementById('preorder'); if (!e) return;
    // The Squarespace section around this block keeps no padding of its own, so the page starts right under the header.
    var sec = e.closest('section');
    if (sec) Array.prototype.forEach.call(sec.querySelectorAll('.content-wrapper, .fluid-engine, .section-border + .content-wrapper'), function (x) { x.style.setProperty('padding-top', '0', 'important'); x.style.setProperty('padding-bottom', '0', 'important'); });
...
// Region buttons scroll within the page (a plain #link would leave it, because the site sets a base address).
document.querySelectorAll('#preorder .po-jump a').forEach(function (a) {
