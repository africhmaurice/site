...
const WALMART_EXCLUSIVE = 'https://www.walmart.com/ip/Cello-s-Gate-Walmart-Exclusive-Hardcover-9781668266922/20417514704';
...
<style>
#preorder{box-sizing:border-box;background:#0c2308;color:#fffffe;font-family:'Almarai',sans-serif;letter-spacing:.03em;padding-bottom:110px}
#preorder *,#preorder *::before,#preorder *::after{box-sizing:border-box}
#preorder .po-banner{height:clamp(160px,24vw,340px);background:linear-gradient(rgba(12,35,8,.15),#0c2308),url(${BASE}assets/bg/city-skyline-panorama-as348070597.webp) center 40%/cover no-repeat}
#preorder .po-hero{max-width:1180px;margin:clamp(-150px,-9vw,-60px) auto 0;padding:0 32px;display:grid;grid-template-columns:minmax(0,340px) minmax(0,1fr);gap:56px;align-items:center;position:relative}
...
#preorder h1{font-family:'Atomic Marker',cursive;font-weight:400;font-size:clamp(42px,5.2vw,78px);line-height:1;margin:0 0 20px;color:#fffffe;text-transform:none}
#preorder .po-lede{font-size:clamp(16px,1.4vw,19px);line-height:1.6;color:#ede9dc;margin:0 0 28px;max-width:560px}
#preorder .po-excl{display:inline-flex;flex-direction:column;gap:6px;border:2px solid #f3ead9;background:rgba(193,51,10,.18);padding:18px 24px;text-decoration:none;color:#fff;transition:transform .15s ease,background .15s ease}
#preorder .po-excl:hover{background:#c1330a;transform:scale(1.03)}
#preorder .po-excl b{font-weight:800;font-size:17px;letter-spacing:.08em}
#preorder .po-excl span{font-size:14px;color:#ede9dc}
...
<div id="preorder">
  <div class="po-banner" role="img" aria-label="A city skyline of Imperia"></div>
  <div class="po-hero">
    ...
    <div>
      ...
      <p class="po-lede">The deluxe hardcover, ebook and audiobook arrive November 3, 2026. Pick your region below, then your favorite store.</p>
      <a class="po-excl" href="${WALMART_EXCLUSIVE}" target="_blank" rel="noopener"><b>WALMART EXCLUSIVE EDITION →</b><span>A special hardcover you’ll only find at Walmart (U.S.)</span></a>
    </div>
  </div>
...
console.log(`preorder.html: ${RETAILERS.length} links + Walmart Exclusive across ${REGIONS.length} regions`);
